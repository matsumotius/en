$(function(){
    ...
    En.prototype.draw = function(){
        ...
        if(this.pie_array != null && this.pie_array.length > 0){
            for(var i=0;i<this.pie_array.length;i++) this.fill_pie(i);
        }
        if(this.has_image) this.draw_image();
        if(this.text) this.fill_text();
    };
    En.prototype.to_pie = function(pies, start){
        ...
        if(typeof pies == 'number'){
            ...
            for(var i=0;i<pies.length;i++){
                this.pie_array.push({ size : pies[i] / 100, color : this.color });
            }
        }
    };
    En.prototype.fill_pie = function(index, color){
        if(this.pie_array == null || this.pie_array.length < 1) return;
        if(color) this.pie_array[index].color = color;
        var start = 0;
        for(var i=0;i<index;i++) start += this.pie_array[i].size;
        var begin = Math.PI * (- 0.5 + this.pie_start + 2 * start);
        var close = Math.PI * (- 0.5 + this.pie_start + 2 * (start + this.pie_array[index].size));
        this.context.beginPath();
        this.context.moveTo(this.x, this.y);
        this.context.arc(this.x, this.y, this.radius, begin, close, false);
        this.context.lineTo(this.x, this.y);
        this.context.closePath();
        this.context.fillStyle = this.pie_array[index].color;
        this.context.fill();
    };
    En.prototype.set_text = function(text, x, y, font){
        if(typeof text != 'string' || typeof x != 'number' || typeof y != 'number') return;
        this.text = { value : text, x : x, y : y, font : font ? font : '10px Arial' };
        this.fill_text();
    };
    En.prototype.fill_text = function(){
        if(null == this.text) return;
        var dx = this.x + this.text.x;
        var dy = this.y - this.text.y;
        this.context.font = this.text.font;
        this.context.fillText(this.text.value, dx, dy);
    };
