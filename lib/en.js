$(function(){
    ...
    EN_EVENTS = ['click', 'mousemove', 'mouseup', 'mousedown', 'mouseover', 'dblclick'];
    var En = function(_target, _options){
        ...
        this.rail = null;
        this.events = {};
        for(var i=0;i<EN_EVENTS.length;i++) this.events[EN_EVENTS[i]] = [];
        this.draw();
    };
    ...
    En.prototype.get_angle = function(e){
        ...
        var radian = Math.atan2(this.y - e.y, e.x - this.x);
    ...
    EnManager.prototype.apply_events = function(target){
        ...
        $.each(EN_EVENTS, function(index, en_event){
            $(target)[en_event](function(e){
                var that = this;
                var en_list = null;
                var x = 0, y = 0;
                ...
                $.each(en_list, function(index, en){
                    var distance = Math.pow(x + en.x - e.pageX, 2) + Math.pow(y + en.y - e.pageY, 2);
                    if(distance <= Math.pow(en.radius, 2)) en.fire(en_event, e);
                });
            });
        });
    };
